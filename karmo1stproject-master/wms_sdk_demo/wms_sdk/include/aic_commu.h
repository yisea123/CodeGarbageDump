...
enum class EXPORT_CLASS AicCommuStatus {
  ...
  TIMEOUT,        // 超时(仅用于request模式, request模式下超时会自动重连,
                  // 在重连过程中, 不会发送DISCONNECTED事件, 但会发送CONNECTED事件)
};
 ...
 * @param 参数一           枚举的状态类型
 * @param 参数二
 * 与socket相关的地址信息,对于服务端socket类型,该参数是客户端的地址; 对于客户端socket类型,该参数是服务端地址
 * @return
 */
 ...
 * @return
 */
using RecvCall = std::function<void(_IN_ const std::string *, _IN_ bytes_ptr, _OUT_ bytes_ptr)>;
...
class EXPORT_CLASS AicCommuInterface {
 public:
   ...
   * @param discard_before_connected  该参数只有请求模式socket有效,
   *                                  true: 在与服务端建立连接前丢弃所有发送请求;
   *                                  false: 不管有没有连接成功, 都把发送请求加入发送队列里
   * @return                   true: 已加入发送队列; false: 失败
   */
  virtual bool send(bytes_ptr buffer, RecvCall func = nullptr, bool discard_before_connected = false) = 0;
   ...
   * @return
   */
  virtual void setLogCall(LogCall func, AicCommuLogLevels level = AicCommuLogLevels::DEBUG,
                          bool is_log_time = false) = 0;
...
class EXPORT_CLASS AicCommuFactory {
 public:
   ...
   */
  static std::shared_ptr<AicCommuInterface> newSocket(AicCommuType type, const std::string &ip, const unsigned int port,
   ...
   */
  static bytes_ptr makeBytesPtr(const char *data, int size);
